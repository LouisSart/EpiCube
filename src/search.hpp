...
Solutions<NodePtr>
depth_first_search(const NodePtr root, const Mover &apply,
                   const Pruner &estimate, const SolveCheck &is_solved,
                   const Directions &directions, const unsigned max_depth = 4) {
...
template <bool verbose = true, typename NodePtr, typename Mover,
          typename Pruner, typename SolveCheck, typename Directions>
Solutions<NodePtr>
IDAstar(const NodePtr root, const Mover &apply, const Pruner &estimate,
        const SolveCheck &is_solved, const Directions &directions,
  ...
  while (solutions.size() == 0 && search_depth <= max_depth) {
    ...
    solutions = depth_first_search<verbose>(root, apply, estimate, is_solved,
                                            directions, search_depth);
    ++search_depth;
  }
  if (slackness > 0) {

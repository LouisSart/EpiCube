...
struct CubieCube {
  ...
  void corner_apply(const CubieCube &cc) {
    ...
    std::array<Cubie, NC> new_cp;
    std::array<Cubie, NC> new_co;
  ...
  static CubieCube random_state() {
    ...
    std::vector<Cubie> corners_left{ULF, URF, URB, ULB, DLF, DRF, DRB, DLB};
    for (Cubie c = ULF; c < NC; ++c) {
...
bool operator==(const CubieCube &cc1, const CubieCube &cc2) {
  for (Cubie c = ULF; c <= DLB; ++c) {
    if (cc1.cp[c] != cc2.cp[c])
      return false;
    if (cc1.co[c] != cc2.co[c])
      return false;
  }
  for (Cubie e = UF; e <= DL; ++e) {
    if (cc1.ep[e] != cc2.ep[e])
      return false;
    if (cc1.eo[e] != cc2.eo[e])
      return false;
  }
  return true;
}
...
CubieCube CubieCube::get_conjugate(const unsigned &sym_index) const {
...
CubieCube CubieCube::get_anti_conjugate(const unsigned &sym_index) const {

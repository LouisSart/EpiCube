...
public:
  ...
  std::vector<sptr> expand(const F &apply, const MoveContainer &directions) {
    ...
    for (auto &&move : directions) {
      next = state;
      apply(move, next);
      children.emplace_back(new Node(next, depth + 1, this->shared_from_this(),
                                     {move}, step_number));
    }
    return children;
  };
  ...
  auto get_skeleton(std::vector<std::string> comments) const {
    ...
    while (!p->is_root()) {
      if (p->parent->step_number == cur_step) {
        ...
        skeleton.push_back(StepAlgorithm(step_path, comments[cur_step]));
        cur_step = p->parent->step_number;
...
template <typename Cube> typename Node<Cube>::sptr make_root(const Cube &cube) {
